...
)
{
    ...
    if (height == 0) {        
        ...
        {
            // Hexagon nut
            hexNut(diameter = mThread[3] + additionalWidth, height = mThread[4]);
            
            // Innerthread
            if (hasThread) {
                translate([0,0,-0.1]) {
                    if (debug) {
                        cylinder(mThread[4] + internalThreadTolerance, r=mThread[1]/2, $fn=64);
                    } else {
                        metricThread(name, mThread[4] + internalThreadTolerance, innerThread=true); 
                    }
                }
            }
        }  
        ...
        {
            // Hexagon nut
            hexNut(diameter = mThread[3] + additionalWidth,height = height);
            
            // Innerthread
            if (hasThread) {
                translate([0,0,-0.1])	metricThread(name, height+0.2, innerThread=true);            
            }
        }  
    }
}
...
)
{
    ...
    m_pitchRadius = tThread[2]*2 + + internalThreadTolerance;
